...
export default class BleHelper {
    ...
    getUUID(peripheralInfo) {
        this.getNewUUID(peripheralInfo);
        this.readServiceUUID = [];
        this.readCharacteristicUUID = [];
        this.writeWithResponseServiceUUID = [];
        this.writeWithResponseCharacteristicUUID = [];
    ...
    getNewUUID(peripheralInfo) {

        for (let i = 0; i < peripheralInfo.characteristics.length; i++) {

            for (let j = 0; j < peripheralInfo.services.length; j++) {
                let tempArr = (peripheralInfo.services[j].characteristics ? peripheralInfo.services[j].characteristics : [])

                if (peripheralInfo.characteristics[i].service == peripheralInfo.services[j].uuid) {

                    let characteristic = peripheralInfo.characteristics[i];
                    characteristic.propertiesArray = [];

                    if (Platform.OS == 'android') {
                        if (characteristic.properties.Notify == 'Notify') {
                            characteristic.propertiesArray.push('Notify');
                        }
                        if (characteristic.properties.Read == 'Read') {
                            characteristic.propertiesArray.push('Read');
                        }
                        if (characteristic.properties.Write == 'Write') {
                            characteristic.propertiesArray.push('Write');
                        }
                        if (characteristic.properties.WriteWithoutResponse == 'WriteWithoutResponse') {
                            characteristic.propertiesArray.push('WriteWithoutResponse');
                        }
                        if (characteristic.properties.Indicate == 'Indicate') {
                            characteristic.propertiesArray.push('Indicate');
                        }
                    } else {  //ios
                        for (let property of characteristic.properties) {
                            if (property == 'Notify') {
                                characteristic.propertiesArray.push('Notify');
                            }
                            if (property == 'Read') {
                    ...
                    tempArr.push(characteristic);
                    peripheralInfo.services[j].characteristics = tempArr;
                    peripheralInfo.services[j].expanded = false;
                }
            }
    ...
    connect(id) {
        ...
        return new Promise((resolve, reject) => {
            BleManager.connect(id)
                ...
                .then((peripheralInfo) => {
                    console.log('Connected peripheralInfo: ', JSON.stringify(peripheralInfo));
            ...
            BleManager.retrieveServices(this.peripheralId)
                .then(() => {
                    BleManager.startNotification(this.peripheralId, this.nofityServiceUUID[index], this.nofityCharacteristicUUID[index])
                        ...
                        .catch((error) => {
                            console.log('Notificration eror:', error);
                            reject(error);
                        });
                })
                .catch((error) => {
    ...
    startNotificationNew(serviceId, characteristicId) {
        return new Promise((resolve, reject) => {
            BleManager.retrieveServices(this.peripheralId)
                .then(() => {
                    BleManager.startNotification(this.peripheralId, serviceId, characteristicId)
                        .then(() => {
                            console.log('Notification started');
                            resolve();
                        })
    ...
    write(data, index = 0) {
        ...
        return new Promise((resolve, reject) => {
            BleManager.retrieveServices(this.peripheralId)
                .then(() => {
                    BleManager.write(this.peripheralId, this.writeWithResponseServiceUUID[index], this.writeWithResponseCharacteristicUUID[index], stringToBytes(data))
                        .then(() => {
    ...
    writeNew(data, serviceId, characteristicId) {
        // data = this.addProtocol(data);   //在数据的头尾加入协议格式，如0A => FEFD010AFCFB，不同的蓝牙协议应作相应的更改
        return new Promise((resolve, reject) => {
            BleManager.retrieveServices(this.peripheralId)
                .then(() => {
                    BleManager.write(this.peripheralId, serviceId, characteristicId, stringToBytes(data))
                        .then(() => {
                            console.log('Write success: ', data.toString());
                            resolve();
                        })
    ...
    readNew(serviceId, characteristicId) {
        return new Promise((resolve, reject) => {
            BleManager.retrieveServices(this.peripheralId)
                .then(() => {
                    BleManager.read(this.peripheralId, serviceId, characteristicId)
                        .then((data) => {
                            const str = this.byteToString(data);
                            console.log('Read: ', data, str);
                            resolve(str);
